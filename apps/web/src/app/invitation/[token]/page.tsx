...
import { Alert, Button, Card, CardBody, Field, Skeleton } from '@/components/ui';
...
export default function InvitationPage() {
  ...
  useEffect(() => {
    apiFetch<IInvitationPreview>(`/auth/invitation/${token}`)
      .then(setPreview)
      ...
      if (caught instanceof ApiError && caught.fields?.length) {
        ...
        setErrors(
          caught.fields.map((field) =>
            tErrors(field.code.split('.').pop() ?? 'minLength', { min: 10 }),
          ),
        );
      } else {
